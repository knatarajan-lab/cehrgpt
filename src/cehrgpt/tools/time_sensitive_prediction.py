...
def convert_to_concept_probabilities(
    ...
    for concept, count in concept_counts.items():
        ...
        concept_probabilities.append(
            ConceptProbability(
                concept=concept, probability=probability, num_of_simulations=count
            )
        )
...
class TimeSensitivePredictionModel:
    ...
    def simulate(
        self, partial_history: Union[np.ndarray, List[str]], max_new_tokens: int = 0
    ) -> List[List[str]]:
        ...
        self.generation_config.max_new_tokens = (
            self.model.config.n_positions - seq_length
            if max_new_tokens == 0
            else max_new_tokens
        )
    ...
    def predict_time_to_next_visit(
        self, partial_history: Union[np.ndarray, list]
    ) -> TimeToEvent:
        sequences = self.simulate(partial_history=partial_history, max_new_tokens=1)
        return self.extract_time_to_next_visit(partial_history, sequences)

    def predict_next_visit_type(
        self, partial_history: Union[np.ndarray, list]
    ) -> List[ConceptProbability]:
        sequences = self.simulate(partial_history=partial_history, max_new_tokens=3)
        return self.extract_next_visit_type(partial_history, sequences)

    def predict_events(
        self, partial_history: Union[np.ndarray, list], only_next_visit: bool = True
    ) -> List[ConceptProbability]:
    ...
    def extract_time_to_next_visit(
        partial_history: Union[np.ndarray, List[str]], simulated_seqs: List[List[str]]
    ) -> TimeToEvent:
        seq_length = len(partial_history)
        ...
        for simulated_seq in simulated_seqs:
            ...
            for next_token in simulated_seq[seq_length:]:
                if is_att_token(next_token):
                    all_valid_time_intervals.append(
                        extract_time_interval_in_days(next_token)
                    )
                    break
        ...
        sorted_probability_table = [
            ...
            for k, v in sorted(
                probability_table.items(), key=lambda x: x[1], reverse=True
            )
        ]
    ...
    def extract_next_visit_type(
        partial_history: Union[np.ndarray, list], simulated_seqs: List[List[str]]
    ) -> List[ConceptProbability]:
...
def main(args):
    ...
    time_sensitive_prediction_output_folder_name = os.path.join(
        args.output_folder, folder_name, "time_sensitive_predictions"
    )

    next_visit_type_prediction_folder_name = os.path.join(
        args.output_folder, folder_name, "next_visit_type_predictions"
    )

    code_predictions_output_folder_name = os.path.join(
        args.output_folder, folder_name, "code_predictions"
    )
    ...
    LOG.info(
        f"Write time sensitive predictions to {time_sensitive_prediction_output_folder_name}"
    )
    ...
    def filter_func(examples):
        return [
            cehrgpt_model.config.n_positions >= _ >= args.min_num_of_concepts
        ...
        for index, concept_id in enumerate(seq):
            ...
            if is_visit_end(concept_id):
                ...
                if seq_length - 1 > index > seq_length // 2:
                    ...
                    simulated_seqs = ts_pred_model.simulate(
                        partial_history, max_new_tokens
                    )

                    # Extract the predictions for time to the next visit
                    if is_att_token(seq[index + 1]):
                        tte = ts_pred_model.extract_time_to_next_visit(
                        ...
                        tte_visit_output.append(
                            {
                                ...
                                "time_to_next_visit_label": extract_time_interval_in_days(
                                    seq[index + 1]
                                ),
                    ...
                    if index + 3 < seq_length and is_visit_start(seq[index + 2]):
                        ...
                        visit_concept_probs = ts_pred_model.extract_next_visit_type(
                            partial_history, simulated_seqs
                        )
                    ...
                    future_event_predictions = ts_pred_model.extract_events(
                        partial_history, simulated_seqs, only_next_visit=False
                    )
                    future_event_predictions = sorted(
                        map(asdict, future_event_predictions),
                        key=lambda v: v["probability"],
                        reverse=True,
                    )
                    future_event_labels = ts_pred_model.extract_events(
                        partial_history, [seq], only_next_visit=False
                    )
                    future_event_labels = sorted(
                        map(asdict, future_event_labels),
                        key=lambda v: v["probability"],
                        reverse=True,
                    )
            ...
            if len(tte_visit_output) >= args.buffer_size:
                ...
                pd.DataFrame(
                    ...
                    columns=[
                        "person_id",
                        "visit_counter",
                        "time_to_next_visit_label",
                        "time_to_next_visit_average",
                        "time_to_next_visit_median",
                        "time_to_next_visit_std",
                        "time_to_next_visit_most_likely",
                        "time_interval_probability_table",
                        "time_to_next_visit_simulations",
                    ],
                ).to_parquet(
                    os.path.join(
                        time_sensitive_prediction_output_folder_name,
            ...
            if len(next_visit_prediction_output) >= args.buffer_size:
                LOG.info(
                    f"{datetime.datetime.now()}: Flushing next visit type predictions to disk"
                )
                pd.DataFrame(
                    next_visit_prediction_output,
                    columns=[
                    ...
                    os.path.join(
                        next_visit_type_prediction_folder_name,
                        f"{uuid.uuid4()}.parquet",
                    )
                )
            ...
            if len(code_prediction_output) >= args.buffer_size:
                LOG.info(
                    f"{datetime.datetime.now()}: Flushing code predictions to disk"
                )
                pd.DataFrame(
                    code_prediction_output,
                    columns=[
                    ...
                    os.path.join(
                        code_predictions_output_folder_name, f"{uuid.uuid4()}.parquet"
                    )
                )
    ...
    if len(tte_visit_output) > 0:
        LOG.info(
            f"{datetime.datetime.now()}: Flushing time to visit predictions to disk at Final Batch"
        )
        pd.DataFrame(
            tte_visit_output,
            columns=[
            ...
            os.path.join(
                time_sensitive_prediction_output_folder_name,
                f"{uuid.uuid4()}-last.parquet",
            )
        )

    if len(next_visit_prediction_output) > 0:
        LOG.info(
            f"{datetime.datetime.now()}: Flushing next visit type predictions to disk at Final Batch"
        )
        pd.DataFrame(
            next_visit_prediction_output,
            columns=[
            ...
            os.path.join(
                next_visit_type_prediction_folder_name, f"{uuid.uuid4()}-last.parquet"
            )
        )

    if len(code_prediction_output) > 0:
        LOG.info(
            f"{datetime.datetime.now()}: Flushing code predictions to disk at Final Batch"
        )
        pd.DataFrame(
            code_prediction_output,
            columns=["person_id", "visit_counter", "code_labels", "code_predictions"],
        ).to_parquet(
            os.path.join(
                code_predictions_output_folder_name, f"{uuid.uuid4()}-last.parquet"
            )
        )
