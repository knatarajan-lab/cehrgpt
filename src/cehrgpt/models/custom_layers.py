...
class GPT2GraphAttention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        self.register_buffer(
            "bias",
            torch.concat([torch.tril(torch.ones((max_positions, max_positions), dtype=torch.bool)), torch.zeros((max_positions, 1), dtype=torch.bool)], 1).view(
                1, 1, max_positions, max_positions + 1
            ),
            persistent=False,
        )
...
class GPT2GraphBlock(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.attn = GPT2GraphAttention(config, layer_idx=layer_idx)
        self.ln_2 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)

        if config.add_cross_attention:
            self.crossattention = GPT2GraphAttention(config, is_cross_attention=True, layer_idx=layer_idx)
